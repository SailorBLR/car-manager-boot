...
taskManagerModule.controller('carManagerController', function ($scope,$http) {
	
	var urlBase="http://localhost:8080/CarManager";
	$scope.toggle=true;
	$scope.selection = [];
	...
	$http.get(urlBase+'/cars').
    	success(function(data) {
	        $scope.cars = data;
	        for(var i=0;i<$scope.cars.length;i++){
	            if($scope.cars[i].taskStatus=='COMPLETED'){
	           	 $scope.selection.push($scope.cars[i].carId);
	        }
	        }
    });
